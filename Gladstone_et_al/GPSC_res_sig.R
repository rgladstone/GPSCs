...
pre_pen_collection <-  table(subset(T1, Vaccine_Period=="Pre-PCV" , c(Predicted_PEN_MIC_CLSI_meningitis)))

pre_pen_GPSC <-  table(subset(T1, Vaccine_Period=="Pre-PCV" & GPSC.type=="Dominant", c(GPSC,Predicted_PEN_MIC_CLSI_meningitis)))
 
pen_GPSCresults <- matrix(data=NA,nrow=0,ncol=4)
 
 for (x in row.names(pre_pen_GPSC)){
   fishers <- "NA"
   pen_GPSC_by2 <- matrix(c(pre_pen_GPSC[x,1],pre_pen_GPSC[x,2],pre_pen_collection[1]-pre_pen_GPSC[x,1],pre_pen_collection[2]-pre_pen_GPSC[x,2]), byrow = TRUE, 2,2)
 ...
 for (x in row.names(pre_MDR_GPSC)){
   fishers <- "NA"
   MDR_GPSC_by2 <- matrix(c(pre_MDR_GPSC[x,2],pre_MDR_GPSC[x,1],pre_MDR_collection[2]-pre_MDR_GPSC[x,2],pre_MDR_collection[1]-pre_MDR_GPSC[x,1]), byrow = TRUE, 2,2)
